...
class TenderCreatePage extends BasePage {
    ...
    elements = {
        ...
        findServiceInput: () => cy.get('[data-testid="input-customSelectWithSearch"]'),
        ...
        dateBeginningData: () => cy.get('.react-datepicker-wrapper div input[type="text"]').eq(0),
        dateEndingBtn: () => cy.get('[data-testid="datePicker"]').eq(1),
        dateEndingData: () => cy.get('.react-datepicker-wrapper div input[type="text"]').eq(1),
        datePeriodOfWorkBtn: () => cy.get('[data-testid="datePicker"]').eq(2),
        dateNextMonthBtn: () => cy.get('button[aria-label="Next Month"]'),
        dateCalendarTimeBtn: () => cy.get('.react-datepicker__time-list-item--selected'),

        budgetInput: () => cy.get('.CreateItemPrice_flexRow__AFuyx > [data-testid="customInputWrapper"] > :nth-child(2) > [data-testid="custom-input"]'),
        ...
        tenderNameInputErrorMsg: () => cy.get('[data-testid="descriptionError"]').eq(0),
        findServiceInputErrorMsg: () => cy.get('div.CustomSelectWithSearch_errorTextVisible__B5lZH'),
        findServiceModalErrorMsg: () => cy.get('[data-testid="p2-notFound-addNewItem"]'),
        dateEndingErrorMsg: () => cy.get('div.DateContainer_errorTextVisible__of2cc'),
        budgetInputErrorMsg: () => cy.get('[data-testid="descriptionError"]').eq(1),
        placeWorkMapErrorMsg: () => cy.get('.AddressSelectionBlock_errorTextVisible__IAGKS'),
        additionalInformationErrorMsg: () => cy.get('[data-testid="textAreaError"]'),
    ...
    validateAllErrorMsg() {
        const errors = [
            { element: this.elements.tenderNameInputErrorMsg, expectedText: 'Назва має містити щонайменше 10 символів' },
            { element: this.elements.findServiceInputErrorMsg, expectedText: 'Це поле обов’язкове' },
            { element: this.elements.dateEndingErrorMsg, expectedText: 'Це поле обов’язкове' },
            { element: this.elements.budgetInputErrorMsg, expectedText: 'Це поле обов’язкове' },
            { element: this.elements.placeWorkMapErrorMsg, expectedText: 'Виберіть коректне місце на мапі України' },
            { element: this.elements.additionalInformationErrorMsg, expectedText: 'Опис має містити щонайменше 40 символів (0)' }
        ];
    ...
    selectDateEnding(day) {
        this.elements.dateEndingBtn().click();
        cy.get(`.react-datepicker__day--${day.toString().padStart(3, '0')}`).click();
        this.elements.dateCalendarTimeBtn().click();
    }

    selectDatePeriodOfWorkBtn(startDay) {
        this.elements.datePeriodOfWorkBtn().click();
        cy.get(`.react-datepicker__day--${startDay.toString().padStart(3, '0')}`).click();
        const endDay = startDay + 2;
        cy.get(`.react-datepicker__day--${endDay.toString().padStart(3, '0')}`).click();
    }
    ...
    fillAllFields() {
        ...
        this.elements.additionalInformationInput().type('Цей тендер надає послуги з перевезення зерна');
    }
};
